...
- **Cyrius pin**: 6.6.6 (`cyrius.cyml`; the pin selects both the compiler and the stdlib snapshot — every hop is a CHANGELOG entry)
- **Version**: SemVer; `VERSION` is the source of truth and `scripts/version-bump.sh` propagates it. The 2.0 handler ABI (`fn h(args, claims) → result`) is stable across the 2.x→3.x line; patch releases add capabilities, not shape changes. What each release did is in CHANGELOG; what is next is in `docs/development/roadmap.md` (forward-facing only)
...
| Dep | Role |
|-------|------|
| libro 2.10.3 | Hash-linked audit chain (`[deps.libro]` git pin). **patra 1.14.3** (`lib/patra.cyr`, the audit store) arrives via libro's own `deps.patra` manifest block — *not* via `dist/libro.deps`, which lists only stdlib leaves. As of 3.3.10 that pin is level with the 6.6.6 fold; `cyrius deps` keeps the snapshot's copy and warns `refusing to overwrite stdlib leaf 'patra'`, which is the expected shape, not a defect. ⚠ **libro also carries a thin `deps.sigil` selection** (`dist/sigil-mldsa.cyr` + `src/sha256.cyr` / `sha_ni.cyr` / `hex.cyr`) that lands in bote's `lib/` as `sigil-mldsa.cyr` / `sigil_*.cyr` and **wins** over the fold's `lib/sigil.cyr` for 232 functions under last-definition-wins (232 `duplicate fn` warnings on every build — the known baseline). That is benign only while libro's `deps.sigil` tag equals the fold's `# Version:`; it did not at 3.3.8–3.3.9 (3.12.9 vs 3.12.16/17, diff-proven harmless only after the fact). **Compare the two on every libro or toolchain bump.** ⚠ **Struct-layout contract, and it has now bitten once.** 2.8.4 APPENDED a 5th field to `struct chain` (`streaming`, 32 → 40 B) — appended, so `src/libro_tools.cyr`'s raw +0 offset read was unaffected. 2.8.11 **PREPENDED** `magic` to `struct error` (48 → 56 B), shifting every field +8, and the raw accessors that read it silently returned the wrong fields → SIGSEGV on the tamper-report path (fixed 3.3.4). The rule: a field appended at the END is safe to ignore; a field inserted at the FRONT moves everything. `struct error` is now read through libro's `#derive(accessors)` getters (`error_code` / `error_msg` / `error_index`) rather than offsets; `struct chain` / `struct entry` keep raw offsets, where libro exposes no getters. ⚠ 2.8.11+ also changed the entry-hash preimage and the Merkle construction — see the 3.3.4 CHANGELOG for who must re-anchor (bote itself: nobody) |
| majra 2.9.1 | Pub/sub event publishing (`[deps.majra]` git pin). `pubsub_new` / `pubsub_subscribe` / `pubsub_publish` signatures and the 32-byte hub struct are unchanged since 2.7.0 and `PUBSUB_LAG_BLOCK` stays the default, so wire behaviour is identical. 2.9.0's two **wire breaks** (encrypted-IPC handshake, signed-envelope domain separation) are in the `-backends` / `-signed` profiles bote does not consume; 2.8.0's `ERR_*` → `MAJRA_ERR_*` rename keeps same-value aliases; 2.8.1 fixed borrowed topic keys on the hub bote publishes into; 2.7.1 renamed `_sub_new` → `_majra_sub_new`, the libro collision cyrius 6.5.37 turned into a hard error (3.3.8). Since 2.7.0: no per-publish `map_keys()` allocation, and `pubsub_unsubscribe` + `PUBSUB_LAG_UNSUBSCRIBE` mean an abandoned subscriber cannot wedge a bote publish forever |
| sigil 3.12.18 | Crypto (sha256 / hmac_sha256 / ed25519). ⚠ **No `[deps.sigil]` block — the pin was REMOVED at 3.3.1** and sigil now arrives via the cyrius stdlib fold. The pin had gone stale and was holding bote at 3.12.1, behind two authentication bypasses (3.12.5 PKCS#1 v1.5, 3.12.6 RSA-PSS); re-adding it would also reclassify sigil out of the stdlib leaves in `dist/bote.deps` and break clean-room consumers. **Ordering in `[deps] stdlib` is still load-bearing:** `ct` / `keccak` / `random` must precede `sigil` (single-pass forward refs), and **`thread_local` must precede `sigil`** — without TLS storage the binaries link clean and then SIGILL at first crypto use (exit 132) |
| sakshi 2.5.2 | Tracing / structured logging. ⚠ **No `[deps.sakshi]` block — removed at 3.3.1**; it arrives via the stdlib fold. bote references **zero** sakshi symbols directly; it is in `[deps] stdlib` only because libro links it. Do not remove that `"sakshi"` entry on the assumption libro's sidecar supplies it |
| kavach 3.12.5 | Tool sandboxing (pluggable runner via fn-pointer + ctx adapter). **Not a declared dep** — bote ships the adapter shape only and the consumer wires the backend, so there is no `[deps.kavach]` block. kavach's `ExecResult` still matches what `src/sandbox.cyr` documents, but its `sandbox_exec(sandbox, command)` is 2-arg with no timeout, so a consumer adapter must drop bote's `timeout_ms`. Do not add it as a dep: it pulls ai-hwaccel + samay. (The old "pins sigil 3.11.1, behind bote's" rationale is retired — kavach pins sigil 3.12.16; bote gets 3.12.18 from the fold.) |
...
Three per-transport binaries — a cyrius 5.10.x compile-source-cap workaround that the 6.1.24 cap raise made unnecessary; reconsolidation into one `bote` is a 3.4.x roadmap item:
...
**Targets.** Native x86_64 Linux; **aarch64** Linux (`cyrius build --aarch64`, the full suite runs under `qemu-aarch64`); **agnos** (`cyrius build --agnos` — a static x86-64 `ET_EXEC` over the agnos syscall ABI; the Unix transport fail-closes there because agnos has no `AF_UNIX`, the other five carry traffic). CI cross-builds all three entries for both non-native targets and, since 3.3.12, also compiles every test / bench / fuzz unit for agnos with a zero-`undefined function` bar — cycc lowers an unreachable undefined fn to a `ud2` stub and still exits 0, so "OK" alone is not the bar. Runtime on agnos is proven on the kernel's QEMU image (`BOTE_SELFTEST` + `bote-mcp-smoke.sh` in the agnos repo), not in bote's CI.
...
2. Cleanliness check: `for f in src/*.cyr; do cyrius fmt "$f" --check; done`, `cyrius lint src/main.cyr`, `cyrius vet src/main.cyr`, `cyrius deny src/main.cyr` — `fmt` is per-file with the flag after the path; `cyrius vet` is the include-dependency audit (`cyrius audit` is the toolchain's own self-host gate, not ours); plus the raw-`syscall(` grep CI runs over `src/` + `tests/` + `fuzz/`
...
- **Namespace fn names too, not just constants.** The flat "last definition wins" namespace covers **functions** as well as enum constants, and the winner is decided by include order — which a consumer controls, not bote. Worked example: `src/stream.cyr`'s `cancel_token_new()` collided with `lib/async.cyr:1058`'s from 3.3.0 until **3.3.5**, when the family was renamed `bote_cancel_token_*`. bote's own binaries were fine (`src/` is included after `lib/`, so bote's won) — but a consumer vendoring `dist/bote.cyr` and including `async.cyr` after it silently got async's body, with no diagnostic on their side. ⭐ The rename moved `fn_table` +1: under last-definition-wins the shadowed definition occupied no slot, so the collision was literally hiding a function. **`src/` is warning-clean as of 3.3.5 — keep it that way**, so a new `duplicate fn` warning is signal instead of noise in a known-warning baseline. (Two deps can still collide with each other and bote cannot fix that from here. `_sub_new` — libro vs majra — was one until majra 2.7.1 renamed it; the live case is libro's thin sigil selection vs the fold's `lib/sigil.cyr`, 232 names, see the libro row in the stack table.)
- **Namespace global constants.** Enum constants share one flat global namespace with "last definition wins" — a bare tag silently takes a dep's value if both define it. `BoteErrTag` tags are prefixed `BOTE_ERR_*` (namespaced at 3.1.3) after bare `ERR_IO` / `ERR_JSON` collided with libro's own `ERR_IO=3` / `ERR_JSON=4` in any libro-linked binary. Prefix every new enum/const with `BOTE_` (or a module tag like `SCH_`) so it can't alias a stdlib/dep symbol. **The rule cuts the other way too:** a bare stdlib name bote *reads* can be supplied by a dep on a target where the stdlib doesn't define it. Worked example (3.3.12): `transport_unix.cyr` used bare `EINTR` / `EAGAIN` / `EINVAL` / `EMFILE`; the agnos syscall peer defines no errno names, and the four resolved anyway — from `lib/sigil.cyr:44`'s own bare errno enum in the injected prelude, correct only because sigil spells Linux's numbers. `ENFILE`, absent from sigil's enum, is what finally failed the build. The module now spells all twelve `BOTE_ERRNO_*` it classifies. When a bare stdlib constant compiles on a target whose peer you know lacks it, find out who is really defining it.
- **Compile-source budget.** Watch `cyrius build` for `expanded source exceeds`. It has not fired since the 6.1.24 cap raise, but three response paths are on record if it does: an upstream cap raise, a per-transport binary split (the shape bote still carries from the 5.10.x era), and an opt-in module split for consumers (`dist/bote-core.cyr`).
- **Function-table cap.** CI gates fn_table + identifier-buffer utilisation at < 95% (`CYRIUS_STATS=1`), parsing the cap out of the `cyrius stats:` block rather than hardcoding it. At 3.3.12 (cyrius 6.6.6) `src/main.cyr` sits at `fn_table 5708/131072`, `identifiers 184274/8388608`, `var_table 2763/1048576` — **4.4% / 2.2% / 0.3%**. Two rules: compare percentages only within a toolchain line (6.4.75 / 6.4.76 and the 6.6.x line each raised the ceilings, so older percentages are not comparable), and ⚠ **re-measure both sides on freshly-resolved trees before quoting a delta** — a figure taken against a stale `lib/` is not a baseline. Growth between releases is usually a dep's unreachable set (bayan's, at 3.3.4), not bote.
- **A toolchain codegen A/B IS obtainable — run it on risky bumps.** Per-version toolchains live at `~/.cyrius/versions/<V>/` and **the `cyrius` wrapper re-execs the pinned one** (since 6.6.4: `cyrius which` inside the repo resolves to `~/.cyrius/versions/<pin>/bin/cycc`, not `~/.cyrius/bin/cycc`), so editing the `cyrius =` pin selects both the compiler and the stdlib snapshot. Recipe: build once at the old pin (`git show HEAD:cyrius.cyml > cyrius.cyml`, then `rm -rf lib && cyrius lib sync --full && cyrius deps`), keep that binary, restore the new pin and `lib/`, rebuild, run the two interleaved. ⚠ `path =` siblings vendor their WORKING TREE on both sides, so this is a toolchain control, not a previous-release reproduction, unless the siblings are checked out at the old tags. Done at 3.3.10 for 6.6.3 → 6.6.6 (14 benches, a wash: dispatch −3…−7%, schema +1…+3%), and — by stdin-prelude reconstruction, which the pre-6.6.4 wrapper needed because it ignored the pin for the compiler — at 3.3.4 for the 6.5.35 regalloc rewrite: 867/867 assertions agreed compiler-for-compiler.
- **No `unwrap()` / `panic!()` analog.** Library code returns 0 / -1 / error tags; consumer decides.
- **No raw `syscall(`.** Every kernel touch goes through a stdlib wrapper — `sys_*` from `lib/syscalls*.cyr`, `random_bytes`, `eprint`, `sock_*`. A raw number is one target's ABI (60 is `exit` on x86_64 only; 8 is `lseek` on Linux and `dup` on agnos), and `SYS_ACCEPT` showed a bare constant can be x86-only even when it resolves everywhere. CI bans the form across `src/`, `tests/` and `fuzz/` (comment-stripped) since 3.3.11; `src/` has been clean since 3.2.1. Literals passed *to* a wrapper — mode bits, a backlog, fds 0/1 — are POSIX, not ABI, and are fine.
...
| `tests/bote_transport_unix.tcyr` | 47 | Unix transport — `_unix_sockaddr` (incl. the 107-byte truncation clamp), accept-error policy `_unix_accept_action` + capped backoff, `sleep_ms` really blocks, `sys_accept4` arch guard (that one probe is `#ifndef CYRIUS_TARGET_AGNOS` — the target has no `accept4`, and the loop it exercises is guarded the same way). New at 3.2.1; module had no coverage before |
...
docs/ (required):
  ...
  development/roadmap.md — forward-facing only: next patch, upcoming minors, open decisions, 4.0 criteria (what shipped is the CHANGELOG)
...
- **Do not issue a raw `syscall(`** in `src/`, `tests/` or `fuzz/` — call the stdlib wrapper (`sys_exit`, `sys_open`, `random_bytes`, …). CI fails on the form itself, not on a constant list
...
- **Do not write history into `cyrius.cyml` — it is a manifest, not a ledger.** Present-tense rules only: why an order matters, what must not be added, what a contract is. Incidents and version-by-version narrative go in CHANGELOG; standing rules go in this file; distribution reasoning goes in DEPS-PATTERN.md. No comments inside an array; write `deps.NAME` in prose, never the bracketed header. ⚠ **The failure mode is adding one line of context while making a real change** — a version number, a "joined at X", the measurement behind a decision. If an edit to this manifest mentions a release, delete the sentence and put it where it belongs. (It has accreted twice: rewritten 300 → 174 lines at 3.3.10, trimmed again after 3.3.13 added a profile-decision narrative to it.)
