...
- **Namespace fn names too, not just constants.** The flat "last definition wins" namespace covers **functions** as well as enum constants, and the winner is decided by include order — which a consumer controls, not bote. Live instance: `src/stream.cyr:11`'s `cancel_token_new()` collides with `lib/async.cyr:1058`'s. bote's wins today only because `src/` is included after `lib/`; a consumer that vendors `dist/bote.cyr` and includes `async.cyr` after it silently gets async's version instead. `cyrius build` reports these as `duplicate fn` warnings — read them, they are not noise. (Two deps can collide with each other too: `_sub_new` is defined by both `lib/libro.cyr` and `lib/majra.cyr`.)
