...
| Dep | Role |
|-------|------|
| libro 2.8.4 | Hash-linked audit chain (`[deps.libro]` git pin). Namespaced its own `LibroErr` enum `ERR_* → LIBRO_ERR_*` at 2.8.2 (the upstream reciprocal of bote's `BOTE_ERR_*` — the bare `ERR_IO`/`ERR_JSON` clash is now fixed at the source on both sides). **patra 1.12.12** (`lib/patra.cyr`, the audit store) arrives via libro's own `[deps.patra]` manifest block — *not* via `dist/libro.deps`, which lists only stdlib leaves. bote adapts libro's fn API (`audit_libro` / `libro_tools`), not its error constants, so the rename is transparent. 2.8.4 appends a 5th field to `struct chain` (`streaming`, 32 → 40 bytes) for the new `chain_new_streaming()` write-through constructor — appended, so `src/libro_tools.cyr`'s raw +0 offset read is unaffected |
| majra 2.5.3 | Pub/sub event publishing (`[deps.majra]` git pin). 2.5.3 restricts `#`/`+` to whole-level positions — bote is unaffected (all 11 topics in `src/events.cyr` are fully-qualified 3-level strings; the only pattern in-tree is a whole-level trailing `#`) — and fixes head-of-line blocking where one full subscriber channel froze the hub inside the mutex |
| sigil 3.12.1 | Crypto (sha256 / hmac_sha256 / ed25519) — **`[deps.sigil]` git pin**, NOT the bare `sigil` stdlib entry. The stdlib registry has historically lagged the self-contained bundle (the 6.1.x registry resolved `sigil` to 3.7.10, whose `dist/sigil.cyr` dangled `include "src/sha_ni.cyr"`; 3.7.12+ bundles are self-contained). The pin overrides the registry so `cyrius deps` gets the current self-contained bundle. Full bundle retained over the 3.11.x per-primitive profiles — bote consumes two overlapping primitive families (SHA-2/HMAC + Ed25519), and the thin closures overlap on ~121 fns. **`thread_local` must precede `sigil`** in `[deps] stdlib` — sigil ≥3.7.14's `crypto_scratch` exercises the TLS path and SIGILLs at runtime without it, and 3.12.1 makes that ordering load-bearing a second way by claiming its crypto-bank TLS slot through `thread_local_alloc()` instead of a hardcoded literal |
| sakshi 2.4.7 | Tracing / structured logging — **`[deps.sakshi]` explicit pin** (since 3.1.2): the registry resolves `sakshi` → 2.4.3, which trips the lib-freshness shadow warning on every build. Same registry-lag class as sigil. `dist/sakshi.cyr` is self-contained (links only stdlib `fnptr` + `atomic`). bote references **zero** sakshi symbols directly — it is pinned because libro links it. Declares the highest toolchain floor in bote's dep set (6.5.0) |
| kavach 3.9.3 | Tool sandboxing (pluggable runner via fn-pointer + ctx adapter). **Not a declared dep** — bote ships the adapter shape only and the consumer wires the backend, so there is no `[deps.kavach]` block. Label refreshed from a nine-minor-stale "3.0" at 3.2.0; kavach's `ExecResult` still matches what `src/sandbox.cyr` documents, but note its `sandbox_exec(sandbox, command)` is 2-arg with no timeout, so a consumer adapter must drop bote's `timeout_ms`. Do not add it as a dep: it pins sigil 3.11.1 (behind bote's) and pulls ai-hwaccel + samay |
...
- **Function-table cap.** CI gates fn_table + identifier-buffer utilisation at < 95% (`CYRIUS_STATS=1`). At 3.2.0 (cyrius 6.5.3) we're at **15% / 31%** on `src/main.cyr` (`fn_table 4961/32768`, `identifiers 163966/524288`). **The denominators moved, not the numerators** — 6.4.75 raised the fn_table ceiling 8192 → 32768 (and fixed a P0 where growth past 8192 silently corrupted six fn-indexed side tables *while the capacity warning went quiet*; bote at ~4900 fns was never exposed), and 6.4.76 raised the identifier pool 256 KB → 512 KB in place. Against 3.1.4's `4879/8192` + `161809/262144` (60% / 62%) the real code growth is +82 fns, all from the 6.5.3 stdlib re-sync. The CI gate itself needs no edit — it parses the cap out of the `cyrius stats:` block rather than hardcoding it. Compare percentages only within a toolchain line.
