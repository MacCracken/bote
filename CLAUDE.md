...
- **Cyrius pin**: 6.6.6 (see `cyrius.cyml`; `6.6.3 → 6.6.6` at 3.3.10; `6.6.2 → 6.6.3` at 3.3.9; onto the 6.6.x line (`6.5.35 → 6.6.2`) at 3.3.8; `6.5.31 → 6.5.35` at 3.3.4; `6.5.20 → 6.5.31` at 3.3.2; onto the 6.5.x line at 3.2.0; `6.4.64 → 6.4.66` at 3.1.3; `6.4.34 → 6.4.64` at 3.1.2; onto the 6.4.x line at 3.0.1; `6.3.38 → 6.3.42` at 3.0.0; onto the 6.3.x line at 2.9.0; first 6.2.x at 2.7.6; major jump from 5.10.x at 2.7.3)
- **Version**: SemVer; 2.0 handler ABI (`fn h(args, claims) → result`) stable across the 2.x→3.x line; **3.3.12 current** (agnos portability gate in CI — all three entries + every test / bench / fuzz unit build for `--agnos` with zero undefined functions; the sweep found five test files with an include gap and `transport_unix.cyr`'s bare errno names binding to sigil's enum on agnos, both fixed; the CI test step now fails on a non-zero exit / missing summary / undefined fn. 3.3.11: no raw `syscall(` anywhere in bote's own sources — the last 26 `syscall(SYS_EXIT, …)` sites in tests / bench / fuzz became `sys_exit`, CI bans the form across `src/` + `tests/` + `fuzz/`, and both July issues were re-verified and archived; first complete `qemu-aarch64` sweep, 887 / 887. 3.3.10: toolchain 6.6.6 + libro 2.10.3 / majra 2.9.1, `cyrius.cyml` rewritten as a manifest, and the thin-sigil-vs-fold version rule in the libro row below. 3.3.9 retired the distlib `ulimit` cap; 3.3.8 moved onto 6.6.x + libro 2.10.0 / majra 2.7.2 — the `_sub_new` arity build-stopper — and put fuzz in CI; 3.3.4 fixed a SIGSEGV in `libro_verify` — libro 2.8.11 PREPENDED `magic` to `struct error`, shifting every field +8, and bote's raw-offset accessors read the old layout, so decoding a TAMPERED chain dereferenced an integer error code as a pointer; also repairs `_bote_server_version()`, which had reported 3.3.2 since 3.3.1. libro 2.8.10 at 3.3.3; cyrius 6.5.31 + the transitive patra downgrade at 3.3.2; `sys_accept4` + accept-loop error policy at 3.2.1; libro `LIBRO_ERR_*` namespacing at 3.1.4; `BOTE_ERR_*` error-tag namespacing at 3.1.3; web tools at 3.1.0; full MCP capability suite — prompts / resources / completion + polled list_changed push — at 3.0.0; see CHANGELOG)
...
- **Do not write history into `cyrius.cyml` — it is a manifest, not a ledger.** Present-tense rules only: why an order matters, what must not be added, what a contract is. Incidents and version-by-version narrative go in CHANGELOG; standing rules go in this file. No comments inside an array; write `deps.NAME` in prose, never the bracketed header. (Rewritten at 3.3.10, 300 → 174 lines, after it had accreted six releases of changelog.)
