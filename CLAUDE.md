...
- **Version**: SemVer; 2.0 handler ABI (`fn h(args, claims) → result`) stable across the 2.x→3.x line; 3.1.3 current (toolchain 6.4.66 + `BOTE_ERR_*` error-tag namespacing; full dep refresh at 3.1.2; web tools at 3.1.0; full MCP capability suite — prompts / resources / completion + polled list_changed push — at 3.0.0; see CHANGELOG)
...
- **Function-table cap.** CI gates fn_table + identifier-buffer utilisation at < 95% (`CYRIUS_STATS=1`). At 3.1.3 (cyrius 6.4.66) we're at **60% / 62%** on `src/main.cyr` (`fn_table 4879/8192`, `identifiers 161746/262144`) — the bump over 3.1.2's `4841/160696` (59% / 61%) is the 6.4.66 stdlib re-sync (thread_local_alloc slot allocator + patra/bayan snapshot refresh), not the error-tag rename. Well under the 95% gate and far below the 93% / 92% peak at 2.7.2 on the old 5.10.x caps.
