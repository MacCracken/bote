...
| **Total** | **887** | + 1 drift smoke. Green on **x86_64**. ⚠ On **aarch64** the CROSS-BUILD is gated in CI and green, but the runtime sweep under `qemu-aarch64` 11.1.0 is PARTIAL — qemu does not pass `getrandom` through (`random_bytes(16)` returns a negative errno there, 16 natively), so `bote.tcyr`, `bote_pkce.tcyr` and `bote_streamable.tcyr` exit 90 on bote's fail-closed session-ID contract. 373 of 883 execute, 0 failed. This is an emulator limit, not a bote defect |
