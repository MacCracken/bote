...
- **Cyrius pin**: 6.4.66 (see `cyrius.cyml`; `6.4.64 → 6.4.66` at 3.1.3; `6.4.34 → 6.4.64` at 3.1.2; onto the 6.4.x line at 3.0.1; `6.3.38 → 6.3.42` at 3.0.0; onto the 6.3.x line at 2.9.0; first 6.2.x at 2.7.6; major jump from 5.10.x at 2.7.3)
- **Version**: SemVer; 2.0 handler ABI (`fn h(args, claims) → result`) stable across the 2.x→3.x line; 3.1.4 current (libro 2.8.2 — upstream `LIBRO_ERR_*` namespacing + `[deps.libro]` pin/lock realign; toolchain 6.4.66 + `BOTE_ERR_*` error-tag namespacing at 3.1.3; full dep refresh at 3.1.2; web tools at 3.1.0; full MCP capability suite — prompts / resources / completion + polled list_changed push — at 3.0.0; see CHANGELOG)
...
| Dep | Role |
|-------|------|
| libro 2.8.2 | Hash-linked audit chain (`[deps.libro]` git pin). Namespaced its own `LibroErr` enum `ERR_* → LIBRO_ERR_*` at 2.8.2 (the upstream reciprocal of bote's `BOTE_ERR_*` — the bare `ERR_IO`/`ERR_JSON` clash is now fixed at the source on both sides). Its `.deps` sidecar also pulls **patra 1.12.12** (`lib/patra.cyr`) for the audit store since libro 2.8.x. bote adapts libro's fn API (`audit_libro` / `libro_tools`), not its error constants, so the rename is transparent |
...
- **Function-table cap.** CI gates fn_table + identifier-buffer utilisation at < 95% (`CYRIUS_STATS=1`). At 3.1.4 (cyrius 6.4.66) we're at **60% / 62%** on `src/main.cyr` (`fn_table 4879/8192`, `identifiers 161809/262144`) — flat vs 3.1.3 (libro 2.8.2's body was already the vendored one via the local `path`, so the pin/version bump adds no code). The step up from 3.1.2's `4841/160696` (59% / 61%) was the 6.4.66 stdlib re-sync (thread_local_alloc slot allocator + patra/bayan snapshot refresh), not either error-tag rename. Well under the 95% gate and far below the 93% / 92% peak at 2.7.2 on the old 5.10.x caps.
