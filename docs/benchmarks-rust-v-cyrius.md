...
| | Rust v0.92.0 (final) | Cyrius v1.9.2 (current) |
|---|---|---|
| **Source LOC** (`src/`) | 10,877 | 4,888 — **~2.2× smaller** |
| **Tests + fuzz LOC** (`tests/` + `fuzz/`) | (Rust criterion + tests) | 2,212 |
| **External deps** | ~50 crates (axum, tokio, serde_json, criterion, …) | 0 — vendored stdlib in `lib/` + 2 git-pinned AGNOS deps (`libro`, `majra`) |
| **Binary** (release) | not committed (typical Rust release of this surface: 10–20 MB) | **383 KB**, single static ELF, no libc dependency |
| **Build tool** | `cargo` + `rustc` | `cyrius build` (one shot, sub-second) |
| **Tests** | 248 lib + 44 conformance + 12 doc | 519 unit + 4 fuzz harnesses |
| **Benchmarks** | 13 criterion benches | 10 hot-path benches (`tests/bote.bcyr`) |
| **CPU** | AMD Ryzen 7 5800H | AMD Ryzen 7 5800H (same machine) |
| **Toolchain** | Rust 1.89 | cyrius 4.7.0 |
...
The Cyrius port now covers: **everything in v0.92.0 except threaded
streaming dispatch** (waiting on cyrius thread/async surface), **OAuth
2.1 / PKCE / JWT verifier** (bearer middleware shipped in 1.9.0), and
**kavach sandboxing** (waiting on kavach v2). Plus net-new in Cyrius:
**streamable HTTP**, **WebSocket transport**, **typed content blocks**,
**SSRF guard**, **`BOTE_BEARER_TOKENS` env-driven auth**.
...
tests/bote.bcyr` on cyrius 4.7.0, bote 1.9.2.
...
| Operation | Rust v0.92.0 | Cyrius v1.9.2 | Ratio | Notes |
|---|---|---|:-:|---|
...
| `process_message single` | 1.09 µs | 6 µs (`codec_process_message`) | **~5×** | Full pipeline: parse → dispatch → serialize. Includes JSON parse and response serialize. |
...
| `validate_params 2-required (no schema)` | 38 ns | n/a | n/a | Cyrius always uses compiled schema if attached; the unschemaed fast path mirrors Rust's. |
| `validate_params typed (string + enum + integer-bounds)` | 103 ns | 1 µs (`validate_compiled_simple`) | **~10×** | Cyrius walks JSON via offset-based parsing each call; Rust uses already-decoded `serde_json::Value`. |
...
### Cyrius-only benchmarks

| Operation | Cyrius v1.9.2 | What it measures |
|---|---|---|
| `jsonx_get_str_flat` | 605 ns | Extract `"name"` from a flat JSON object cstr |
| `jsonx_get_raw_nested` | 878 ns | Extract `"arguments"` (a nested object) — slice-based parser respects nested braces |
| `codec_parse_request` | 2 µs | Parse a tools/call request line into `JsonRpcRequest` struct |
| `codec_serialize_response` | 753 ns | Emit a JSON response from `JsonRpcResponse` struct |
...
walks raw bytes for sub-object extraction. The Cyrius approach (slice +
on-demand extract) is potentially faster for large requests where most
fields are ignored — but that case isn't benched yet.
...
3. **No closure-captured handler state.** Rust handlers are
   ...
   Cyrius handlers are bare function pointers (`fn h(args_cstr) →
   result_cstr`); state is threaded through globals or args. Fine for
   typical tools but means slightly more bookkeeping per dispatch.
...
| | Rust v0.92.0 | Cyrius v1.9.2 |
|---|---|---|
| Binary size | ~10–20 MB (typical release with this surface) | **383 KB** |
| Build time | ~30 s clean release (cargo + rustc) | **<1 s** (cyrius build) |
...
| Source LOC | 10,877 | **4,888 (2.2× smaller)** for ~equal feature surface |
| Test count | 304 | **519** (1.7× more) |
...
- **OAuth 2.1 / PKCE / JWT verifier** — Rust shipped these; Cyrius has the bearer-token substrate (1.9.0) and will layer the rest on top later.
- **Tooling** — clippy, rust-analyzer, `cargo bench --baseline X`, criterion's variance reporting.

---

## Net call

The port shipped at **2.2× less source code** for a surface that's now
broader than the Rust v0.92.0 baseline (Rust didn't have streamable HTTP,
WebSocket, typed content blocks, SSRF guard, env-driven auth). The
3–10× per-op latency is the price; for sub-1000-req/s deployments — the
realistic target for an MCP service — the **boot time, binary size, and
zero-deps story dominate the per-call latency** by orders of magnitude.
...
- `auth_bearer_check` (1.9.0+) — middleware overhead with allowlist of N
- `ssrf_check` (1.8.0+) — URL classification cost

### Should have added to Rust (n/a — port complete)
- `jsonx`-style raw byte extraction — Rust has no analogue, so the
  comparison was always one-sided. Cyrius's slice-and-cache approach is
  potentially faster for large requests where most fields are ignored,
  but that workload isn't benched on either side.
