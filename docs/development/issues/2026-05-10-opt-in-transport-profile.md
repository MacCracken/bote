...
## ✅ Resolved in 2.7.2 — 2026-05-11

Landed **Option A** (ship `dist/bote-core.cyr` alongside
`dist/bote.cyr`). Shape matches the proposal exactly: 9-module
core, 1989 lines / 70 KB, mechanical `cyrius distlib core` split.
Wire-up:

- **`cyrius.cyml`** — new `[lib.core]` profile listing the nine
  core modules. Default `[lib]` (23 modules → `dist/bote.cyr`)
  unchanged. Comment header documents the two-invocation contract.
- **`dist/bote-core.cyr`** — committed; regenerated by
  `cyrius distlib core`.
- **`tests/bote_core_only_smoke.tcyr`** (new) — drift guard.
  Includes only `dist/bote-core.cyr` plus the minimal stdlib and
  runs a `dispatcher_new + registry_register + dispatcher_handle`
  round-trip. Catches any future bote change that wires a
  core-module symbol against a transport-module helper.
- **`DEPS-PATTERN.md`** (new) — distribution contract doc with
  the "Profile selection" section the proposal recommended, plus
  the core-9 module table.
- **`.github/workflows/ci.yml`** — dist-freshness gate extended
  to diff `dist/bote-core.cyr` byte-for-byte against the
  committed copy; test loop includes `bote_core_only_smoke.tcyr`.
- **CHANGELOG 2.7.2** — full release notes.

Concurrent with this issue, bote 2.7.2 also landed a
**per-transport binary split** inside the repo itself — the
maintenance bump (cyrius 5.10.34 → 5.10.44 + libro 2.6.2 → 2.6.3)
pushed `src/main.cyr`'s own expanded source past the 2 MB cap,
mirroring the consumer-side problem this issue describes. Default
`build/bote` keeps stdio + http + unix + bridge; new
`build/bote-streamable` and `build/bote-ws` siblings carry the
streamable + WebSocket transports. Reconsolidates on cyrius 5.11.x.

**Companion proposal status** — `cyrius/docs/development/proposals/2026-05-10-raise-compile-source-cap.md`
remains open at the time of this resolution. When it lands, the
per-transport binary split inside bote retires; the `dist/bote-core.cyr`
profile stays (still useful for consumers that don't need transports —
faster CI, smaller compile-source budget for their own modules).

**Next consumer action** — t-ron 2.1.x flips its
`[deps.bote] modules = [...nine-cherry-picked-files...]` to
`modules = ["dist/bote-core.cyr"]` in its next patch, closing the
"Future / Blocked" row on the t-ron roadmap.
